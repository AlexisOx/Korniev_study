...
def test_create_car(button=None):
    ...
    WebDriverWait(driver, 3).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, "button[class^='btn btn-primary']"))).click()

    inputElement = driver.find_element(By.CSS_SELECTOR, "select[id^='addCarBrand']")
    inputElement.send_keys("BMW")

    inputElement = driver.find_element(By.CSS_SELECTOR, "select[id^='addCarModel']")
    inputElement.send_keys("X5")

    inputElement = driver.find_element(By.CSS_SELECTOR, "input[id^='addCarMileage']")
    inputElement.send_keys("100")

    WebDriverWait(driver, 3).until(
        EC.element_to_be_clickable((By.CSS_SELECTOR, ".modal-footer > .btn-primary"))).click()

    assert "BMW X5" in driver.page_source
